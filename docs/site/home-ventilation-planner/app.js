(() => {
  ...
  function load(){try{const x=JSON.parse(localStorage.getItem(KEY)||"{}");return {home:x.home||{},hours:Array.isArray(x.hours)?x.hours.filter(valid):[]};}catch{return {home:{},hours:[]};}}
  ...
  function decision(h,x){
    ...
    if(x.smoke==="likely"){classification="Less Favorable";confidence=x.pm25!==null?"High":"Moderate";reasons.push("Likely smoke makes uncontrolled outdoor-air entry unfavorable.");}
  ...
  function renderTable(){
    ...
    table.innerHTML=xs.length?xs.map(x=>`<tr><td>${esc(x.timestamp.replace("T"," "))}</td><td>${num(x.outdoorTemp)}°C</td><td>${x.outdoorHumidity===null?"—":`${num(x.outdoorHumidity,0)}%`}</td><td>${x.pm25===null?"—":num(x.pm25)}</td><td>${x.wind===null?"—":`${num(x.wind)} km/h`}</td><td>${esc(x.smoke)}</td><td>${esc(x.solar)}</td><td>${esc(x.source)}</td><td class="no-print"><button class="danger del" data-id="${esc(x.id)}" style="min-height:36px;padding:6px 9px">Delete</button></td></tr>`).join(""):`<tr><td colspan="9"><div class="empty-state">No hourly values added.</div></td></tr>`;
  }
  function buildPlan(){
    const h=home(),err=validateHome(h);if(err){alert(err);return;}
    const xs=sorted();if(!xs.length){alert("Add or import at least one hourly value.");return;}
    save();
    ...
    result.innerHTML=`
      <div class="result-summary"><span class="kicker">YOUR HOURLY PLAN</span><h2>${favorable.length} more favorable hour${favorable.length===1?"":"s"}</h2><p>${conditional.length} conditional and ${closed.length} less favorable hour${closed.length===1?"":"s"}. Each result uses the same transparent rules.</p><div class="result-meta"><span>Indoor: ${num(h.indoorTemp)}°C</span><span>Cross-ventilation: ${h.cross?"yes":"limited"}</span><span>Hours assessed: ${assessed.length}</span></div></div>
      <div class="decision-grid">
        ...
        <article class="decision-card"><span class="classification insufficient">Limitations</span><h3>Educational defaults</h3><p>Sensor placement, forecast error and local official thresholds can change the real decision.</p></article>
      </div>
  ...
  function exportCsv(){const h=["timestamp","outdoor_temp_c","outdoor_humidity_pct","pm25_ug_m3","wind_speed_kmh","smoke_context","solar_exposure","source"];const rows=sorted().map(x=>[x.timestamp,x.outdoorTemp,x.outdoorHumidity,x.pm25,x.wind,x.smoke,x.solar,x.source].map(csvEscape).join(","));download(`heatsafe-ventilation-hours-${new Date().toISOString().slice(0,10)}.csv`,[h.join(","),...rows].join("\n"),"text/csv;charset=utf-8");}
  function exportJson(){save();download(`heatsafe-ventilation-plan-${new Date().toISOString().slice(0,10)}.json`,JSON.stringify({schema:"heatsafe-ventilation-planner-v1",home:home(),hours:sorted()},null,2),"application/json");}
  function restoreHome(h){if(!h)return;Object.entries(h).forEach(([k,v])=>{const map={indoorTemp:"indoorTemp",indoorHumidity:"indoorHumidity",cross:"crossVentilation",purifier:"purifier",orientation:"orientation",constraints:"constraints"};const f=form.elements[map[k]];if(!f)return;if(k==="cross"||k==="purifier")f.value=v?"yes":"no";else if(v!==null&&v!==undefined)f.value=v;});}
  ...
  document.querySelector("#clear-hours")?.addEventListener("click",()=>{if(state.hours.length&&confirm("Delete all hourly values from this browser?")){state.hours=[];save();renderTable();result.hidden=true;}});
  ...
  document.querySelector("#import-json")?.addEventListener("change",async e=>{const f=e.target.files?.[0];if(!f)return;try{const p=JSON.parse(await f.text());const xs=Array.isArray(p)?p:p.hours;if(!Array.isArray(xs))throw new Error("No hours array.");const clean=xs.filter(valid).slice(0,72);if(!clean.length)throw new Error("No valid hourly rows.");if(confirm(`Import ${clean.length} rows and replace current hourly data?`)){state.hours=clean;if(p.home)restoreHome(p.home);save();renderTable();}}catch(err){alert(`Import failed: ${err.message}`);}finally{e.target.value="";}});
