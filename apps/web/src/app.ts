...
function resultCard(title: string, body: string, tone = "neutral"): string {
...
for (const button of document.querySelectorAll<HTMLButtonElement>(".tabs button")) {
  button.addEventListener("click", () => {
    for (const other of document.querySelectorAll<HTMLButtonElement>(".tabs button")) other.classList.remove("active");
    for (const panel of document.querySelectorAll<HTMLElement>(".panel")) panel.classList.remove("active");
    button.classList.add("active");
    $<HTMLElement>(`#${button.dataset.tab}`).classList.add("active");
  });
}

const ventilationForm = $<HTMLFormElement>("#ventilation-form");
ventilationForm.addEventListener("submit", async (event) => {
  event.preventDefault();
  const target = $<HTMLElement>("#ventilation-result");
  ...
  try {
    ...
    target.innerHTML = resultCard(
      result.decision,
      `<p><strong>Confidence:</strong> ${escapeHtml(result.confidence)}</p>${list(result.reasons)}<p class="fine">${escapeHtml(result.safety_notice)}</p>`,
      tone,
    );
  } catch (error) {
...
profileForm.addEventListener("submit", async (event) => {
  event.preventDefault();
  const target = $<HTMLElement>("#profile-result");
...
const costForm = $<HTMLFormElement>("#cost-form");
costForm.addEventListener("submit", async (event) => {
  event.preventDefault();
  const target = $<HTMLElement>("#cost-result");
...
function syntheticClimateRecords(): any[] {
  const records: any[] = [];
  let seed = 17;
  const random = (): number => {
...
function drawTrend(points: Array<{ year: number; temperature_c: number }>): void {
  const target = $<HTMLElement>("#trend-chart");
...
$<HTMLButtonElement>("#run-trend").addEventListener("click", async () => {
  const target = $<HTMLElement>("#trend-result");
...
type HeatLogRow = Record<string, string>;
const LOG_KEY = "heatsafe-seven-day-log-v1";
const readLog = (): HeatLogRow[] => JSON.parse(localStorage.getItem(LOG_KEY) ?? "[]");
const writeLog = (rows: HeatLogRow[]): void => localStorage.setItem(LOG_KEY, JSON.stringify(rows));

function renderLog(): void {
  const rows = readLog();
  const target = $<HTMLElement>("#log-result");
  if (!rows.length) {
...
const logForm = $<HTMLFormElement>("#log-form");
logForm.addEventListener("submit", (event) => {
  ...
  const row: HeatLogRow = {};
  for (const name of names) row[name] = value(logForm, name);
  const rows = readLog();
  rows.push(row);
...
$<HTMLButtonElement>("#export-log").addEventListener("click", () => {
  const rows = readLog();
  if (!rows.length) return;
  const first = rows[0];
  if (!first) return;
  const headers = Object.keys(first);
  const quote = (entry: string): string => `"${entry.replaceAll('"', '""')}"`;
...
$<HTMLButtonElement>("#clear-log").addEventListener("click", () => {
...
plannerForm?.addEventListener("submit", async (event) => {
  event.preventDefault();
  const target = $<HTMLElement>("#planner-result");
  const now = new Date();
  ...
  try {
    ...
    target.innerHTML = (result as any[]).map((point) => resultCard(
      new Date(point.timestamp).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }),
      `<p><strong>${escapeHtml(point.classification)}</strong></p>${list(point.reasons)}`,
      point.classification === "More Favorable" ? "positive" : point.classification === "Less Favorable" ? "caution" : "conditional",
    )).join("");
  } catch (error) {
    target.innerHTML = errorCard(error);
  }
...
$<HTMLButtonElement>("#run-comparator").addEventListener("click", async () => {
  const target = $<HTMLElement>("#comparator-result");
